...
public class Tuteur extends UnicastRemoteObject implements ITuteur {
	...
	public void setDomaineExpertise(List<String> domaineExpertise) throws RemoteException {
		this.domainesExpertise = domaineExpertise;
	}
	...
	public boolean ajouterRendezVous(IRendezVous rdv) throws RemoteException {
		for (IRendezVous rdvDejaPrevu : rendezVous) {
			if (rdvDejaPrevu.getJour().equals(rdv.getJour()) && rdvDejaPrevu.getHeureDebut() == rdv.getHeureDebut() && rdvDejaPrevu.getHeureFin() == rdv.getHeureFin()) {
				return false;
			}
		}
		rendezVous.add(rdv);
